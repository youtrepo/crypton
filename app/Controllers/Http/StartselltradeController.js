...
class StartselltradeController {
  async start({request,response,auth}){
    try {
      ...
      let balance=await balances.query().where({email:buyer_data.toJSON()[0].email,coin:coin_type.toLowerCase()}).fetch()
      ...
      if (balance.toJSON()[0].value<coin){
        ...
        let trade_data= await trade.create({
          ...
          rate:rate,
          deadline:d,
          buyer:seller_data.toJSON()[0].user,
          seller:buyer_data.toJSON()[0].username
        })
        ...
        //escrow
        await escrow.create({trade_id:trade_id,amount:coin,coin:coin_type,status:'pending',email:buyer_data.toJSON()[0].email})
        await balances.query().where({email:buyer_data.toJSON()[0].email,coin:'btc'}).decrement('value',coin)
...
module.exports = StartselltradeController
